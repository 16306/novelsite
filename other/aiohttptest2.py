...
def find_text(text_url, lastrowid):
    try:
        ...
        pattern1 = re.compile('id="content">(.*?)<a href="http:', re.S)
        title = re.findall(pattern, res)
        content = re.findall(pattern1, res)
        s.close()
        if len(content[0]) > 8000:
            text = content[0][4000:]
            content.append(text)
            content[0] = content[0][0:4000]
            save.addchaper(lastrowid, title[0], content[0])
            save.addchaper(lastrowid + 1, title[0] + "(2)", content[1])
            print(title[0])
            return None
        else:
            save.addchaper(lastrowid, title[0], content[0])
    ...
    try:
        ...
        for i in range(0, max_page_num + 1):
            ...
            if isinstance(con, tuple):
                ...
                sort = save.findsort()
                lastrowid = save.addnovers(sort, novel_name[0], writer_name[0], int(url[-1]))
                for item in chapter_urls:
                    print(item)
